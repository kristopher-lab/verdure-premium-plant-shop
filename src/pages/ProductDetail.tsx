...
import { ChevronLeft, Minus, Plus, Leaf, Sun, Droplets, Shield } from 'lucide-react';
...
function ProductDetail() {
  ...
  // In a real app, you'd fetch by slug. We'll use ID from slug for this mock.
  // This is a simplification. A real backend would have a slug lookup.
        ...
        <div className="py-8 md:py-10 lg:py-12">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
              <img src={product.images[0]} alt={product.name} className="w-full h-auto object-cover rounded-lg shadow-lg aspect-square" />
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                  ...
                  <RadioGroup value={selectedVariantSku} onValueChange={setSelectedVariantSku} className="flex gap-2 mt-2">
                    {product.variants.map(variant => (
                      <div key={variant.sku}>
                        ...
                        <Label htmlFor={variant.sku} className="flex items-center justify-center rounded-md border-2 border-muted bg-popover px-4 py-3 text-sm font-medium hover:bg-accent hover:text-accent-foreground [&:has([data-state=checked])]:border-primary cursor-pointer">
                          {variant.name}
                        </Label>
                      </div>
                    ))}
              ...
              <Button size="lg" className="w-full mt-8 btn-gradient" onClick={handleAddToCart} disabled={!selectedVariantSku}>
                Add to Cart
              </Button>
            </motion.div>
          </div>
